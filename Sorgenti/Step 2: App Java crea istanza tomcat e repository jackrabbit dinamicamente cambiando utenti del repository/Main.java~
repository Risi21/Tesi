...
public class Main {
    ...
    //contiene il percorso alla cartella del repository
    static String repo_path = "";
    //nome repository = prefisso + nome istanza di tomcat associata
   ...
   {
        String cmd = "sudo mkdir " + TOMCAT_PATH + "/" + instance_name;
        System.out.println("\r\nCreo la cartella dell'istanza di Tomcat:\r\n" + cmd);
        ForkProcess(cmd);
        
        cmd = "sudo mkdir " + PATH_BACKUP + "/" + instance_name;
        System.out.println("\r\nCreo la cartella del backup del repoisitory:\r\n" + cmd);
        ForkProcess(cmd);
   ...
   {
        String cmd = "sudo mkdir " + TOMCAT_PATH + "/" + instance_name + "/conf";       
        System.out.println("\r\nCreo sottocartelle conf, logs, temp, webapps, work");
        ForkProcess(cmd);
        
        cmd = "sudo mkdir " + TOMCAT_PATH + "/" + instance_name + "/logs";
        ForkProcess(cmd);
        
        cmd = "sudo mkdir " + TOMCAT_PATH + "/" + instance_name + "/temp";
        ForkProcess(cmd);
        
        cmd = "sudo mkdir " + TOMCAT_PATH + "/" + instance_name + "/webapps";
        ForkProcess(cmd);
       ...
       //cancella il file server.xml copiato prima in /conf
        cmd = "sudo rm " + TOMCAT_PATH + "/" + instance_name + "/conf/server.xml";
        System.out.println("\r\ncancella il file server.xml copiato prima in /conf:\r\n" + cmd);        
        ForkProcess(cmd);
...
"</Server>");
   ...
   public static void UpdateDB(int[] ports) throws SQLException
   {
       //inserisce nome istanza nel database JRSAAS mysql e le 3 porte configurate
        Connection cn = DriverManager.getConnection("jdbc:mysql://" + MYSQL_URL + "/" + MYSQL_DB + "?user=" + USER_JRSAAS + "&password=" + USER_JRSAAS_PWD + "");
   ...
   {
        File theDir = new File(TOMCAT_SCRIPTS);
        // if the directory does not exist, create it
        if (!theDir.exists()) 
        {
            System.out.println("\r\ncreating directory: " + TOMCAT_SCRIPTS + " e i 2 file start_ e stop_ .sh");
            try
            {
                theDir.mkdir();
             } 
...
"./startup.sh");        
        ...
        String cmd = "sudo chmod u+x " + TOMCAT_SCRIPTS + "/start_" + instance_name + ".sh";
        ForkProcess(cmd);
   }
...
"cd $CATALINA_HOME/bin\n" +
"./shutdown.sh\n"
                    + "echo Tomcat " + instance_name + " Stoppped");        
        ...
        String cmd = "sudo chmod u+x " + TOMCAT_SCRIPTS + "/stop_" + instance_name + ".sh";
        ForkProcess(cmd);
   }   
   ...
   static void CreateScriptRestart() throws IOException, InterruptedException
   {
       //stop_<instance_name>.sh
        ...
        try 
        {            
            File bp = new File(TOMCAT_SCRIPTS + "/restart_" + instance_name + ".sh");            
            
            writer = new BufferedWriter(new FileWriter(bp));
            writer.write("sh " + TOMCAT_SCRIPTS + "/stop_" + instance_name + ".sh\n"
                    + "sh " + TOMCAT_SCRIPTS + "/start_" + instance_name + ".sh");        
        ...
        String cmd = "sudo chmod u+x " + TOMCAT_SCRIPTS + "/restart_" + instance_name + ".sh";
        ForkProcess(cmd);
   }        
   ...
   */
   ...
   static void ExportWarFile() throws IOException
   {
       ...
       InputStream stream = Main.class.getResourceAsStream("/export/jackrabbit.war");
        ...
        try 
        {
            resStreamOut = new FileOutputStream(new File(tomcat_webapp_jackrabbit_path + ".war"));
   ...
   static void CreateNewRepo() throws SQLException, IOException, InterruptedException, RepositoryException 
   {
       ...
       //crea cartella repository e cambia proprietario all'utente tomcat7
       CreateRepoFolder();
   ...
   static void CreateRepoFolder() throws IOException, InterruptedException
   {
        //crea cartella repo dentro alla directory del server:
       System.out.println("\r\nCreo la cartella del repository in " + repo_path); 
       boolean repo = (new File(repo_path)).mkdirs();
        if (!repo) 
        {
    ...
    {
        System.out.println("\r\nCreo file" + repo_path + "/bootstrap.properties");
        BufferedWriter writer = null;
        try 
        {            
            
            //create bootstrap.properties file
            File bp = new File(repo_path + "/bootstrap.properties");
...
"repository.home=" + repo_path + "\n" +
"rmi.enabled=true\n" +
"repository.config=" + repo_path + "/repository.xml\n" +
...
"rmi.port=0");
    ...
    {
        System.out.println("\r\nCreo file " + repo_path + "/repository.xml");
        BufferedWriter writer = null;
        try 
        {
            //create repository.xml file
            File bp = new File(repo_path + "/repository.xml");
...
"  <param name=\"url\" value=\"jdbc:mysql://" + MYSQL_URL + "/" + repo_name + "\"/>\n" +
...
"    <param name=\"url\" value=\"jdbc:mysql://" + MYSQL_URL + "/" + repo_name + "\"/> <!-- use your database setup -->\n" +
...
"</Repository>");
    ...
    static void EditWebXml()
    {
        ...
        File toCanc = new File(tomcat_webapp_jackrabbit_path + "/WEB-INF/web.xml");
        toCanc.delete();
        ...
        {
            //create web.xml file
            System.out.println("scrivendo file: " + tomcat_webapp_jackrabbit_path + "/WEB-INF/web.xml");
            File bp = new File(tomcat_webapp_jackrabbit_path + "/WEB-INF/web.xml");
...
"            <param-value>" + repo_path + "/bootstrap.properties</param-value>\n" +
"            <description>\n" +
